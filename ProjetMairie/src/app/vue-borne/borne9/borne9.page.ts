...
@Component({
  selector: 'app-borne9',
  templateUrl: './borne9.page.html',
  styleUrls: ['./borne9.page.scss'],
})
export class Borne9Page implements OnInit {
  ...
  ouvrirPageIndice1(){
    this.indice1 = true;
    this.router.navigate(['borne9/indice1']);
  }
  /*Ouvrir l'indice 2*/

  ouvrirPageIndice2(){
    this.indice2 = true;
    this.router.navigate(['borne9/indice2']);
  }

  /*Ouvrir page transition 10*/
  ...
  /* Traitement de la réponse numero 9 */
  traitementReponse(){
      if((this.indice1 && this.indice2) && (this.reponse == null)){
        this.deuxiemeBorne(9);
      }
      if(this.reponse != null){
      this.reponse = this.reponse.toLowerCase();
      this.reponse = this.reponse.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      if(this.reponse.indexOf("espace") >= 0 && this.reponse.indexOf("nautique") >= 0){
        this.deuxiemeBorne(9);
      }else if(this.reponse.indexOf("espace") >= 0 || this.reponse.indexOf("nautique") >= 0){
        this.showAlert();
      }else{
        this.errorAlert();
      }
