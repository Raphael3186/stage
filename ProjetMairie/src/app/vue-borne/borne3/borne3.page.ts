...
@Component({
  selector: 'app-borne3',
  templateUrl: './borne3.page.html',
  styleUrls: ['./borne3.page.scss'],
})
export class Borne3Page implements OnInit {
  ...
  ouvrirPageIndice1(){
    this.indice1 = true;
    this.router.navigate(['borne3/indice1']);
  }
  /*Ouvrir l'indice 2*/

  ouvrirPageIndice2(){
    this.indice2 = true;
    this.router.navigate(['borne3/indice2']);
  }

  /*Ouvrir page transition 4*/
  ...
  /* Traitement de la réponse numero 3 */
  traitementReponse(){
    if((this.indice1 && this.indice2) && (this.reponse == null)){
      this.deuxiemeBorne(3);
    }
    if(this.reponse != null){
    this.reponse = this.reponse.toLowerCase();
    this.reponse = this.reponse.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

    if(this.reponse.indexOf("chrestias") >= 0 && this.reponse.indexOf("gilet") >= 0){
      this.deuxiemeBorne(3);
    }else if(this.reponse.indexOf("chrestias") >= 0 || this.reponse.indexOf("gilet") >= 0){
      this.showAlert();
    }else{
      this.errorAlert();
    }
